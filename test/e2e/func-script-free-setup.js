...
function buildChromeWebDriver (extPath, opts = {}) {
  ...
  const chromeProfile = "/home/manhal/.config/google-chrome/mm-chrome-profile-metamask-ui-test-after-update-before-locator-b7eae4b"
  const args = [
    `load-extension=${extPath}`,
    `user-data-dir=${chromeProfile}`,
  ]
  if (fs.existsSync(chromeProfile)) {
    fs.removeSync(chromeProfile)
  }
  if (opts.responsive) {
